...
export default function Carousel(
  { items, autoplayInterval = 5, debugHud = false }: Props,
) {
    ...
    >
      ...
      <div className="relative w-full max-w-[1540px] h-[220px] sm:h-[280px] md:h-[320px] lg:h-[360px] flex items-end justify-center overflow-visible">
        {items.map((item, index) => {
          ...
          if (position === "previous") {
            transform =
              "-translate-x-[40%] sm:-translate-x-[50%] md:-translate-x-[60%] lg:-translate-x-[76%]";
            scale = "scale-75";
            zIndex = "z-20";
          ...
          const heightImageClass =
            "h-[188px] sm:h-[248px] md:h-[288px] lg:h-[360px]";
          const heightContainerClass =
            "h-[220px] sm:h-[280px] md:h-[320px] lg:h-[392px]";

          return (
            <div
              className={`cursor-pointer absolute transition-all border border-ca-700 overflow-hidden duration-700 ease-in-out ${scale} ${transform} ${zIndex} ${translateY} ${opacity} ${widthClass} ${heightContainerClass}`}
              onClick={() => handleItemChange(index)}
            >
              {/* Imagem com tamanho fixo */}
              <div
                className={`relative overflow-hidden ${widthClass} ${heightImageClass}`}
              >
                <img
                  src={item.imageUrl}
                  alt={item.title}
              ...
              <div
                className={`w-full md:h-[32px] h-[24px] flex py-2 bg-ca-900 items-center justify-center text-ca-100 border-t border-ca-700`}
              >
                <span className="text-xs sm:text-sm font-normal truncate px-2">
                  {item.title}
                </span>
              </div>
              ...
              <div
                className={`absolute inset-0 bg-ca-900 transition-opacity duration-700 ease-in-out pointer-events-none ${filterOpacity}`}
              >
              </div>
            </div>
          );
      ...
      {debugHud && (
        <div className="md:flex items-center space-x-4 mt-4">
          ...
          >
            {isPaused.value ? <span>▶ Continuar</span> : <span>❚❚ Pausar</span>}
          </button>
            ...
            {items.map((_, idx) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all ${
                  selectedItem.value === idx ? "bg-gray-800 w-4" : "bg-gray-400"
                }`}
